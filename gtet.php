...
$sql_date_batch="SELECT * FROM  batch_details where batch_no='$batch_no'";
...
$printDate=$_POST['newDate'];;
  $update_date_query=mysql_query("update batch_details SET date='$printDate' where batch_no='$batch_no' ");

	  $update_button_query=mysql_query("update batch_details SET status='yes' where batch_no='$batch_no' ");

	$sl=0;
	$sql="SELECT * FROM candidate_details where batch_no='$batch_no'  AND grade!='ABS' AND grade!='F'";
...
/*End od certificate*/
		  ...
		  $change_image_sql="SELECT * FROM certificate_table where sl_no=2";
		  $change_image_result=mysql_query($change_image_sql,$connection);
		  $change_image_details=mysql_fetch_array($change_image_result);
		  $leftLogo=$change_image_details['leftLogo'];
		      $middleLogo=$change_image_details['middleLogo'];
...
<?php
$button_option="SELECT * FROM batch_details WHERE batch_no='$batch_no'";
...
?>
<form name="dateForm" action="gtet.php"?batch_no=<?php echo $batch_no ?>" method="post">
...
<!--
<form action="gtet.php.php?cerID=<?php echo $cerID ?>" method="post" enctype="multipart/form-data" name="form" style="display:inline" >
...
<center>
<form action="gtet.php?batch_no=<?php echo $batch_no ?>" method="post">

<input  hidden="hidden" value="<?php echo $printDate?>" name="newDate">
<input name="submit" hidden="hidden"   type="submit" value="Save Certificates for Batch<?php echo "$batch_no"; ?>"/>
                      
        </center>            
</form>
<?php } else {
	header('location:createBatchCertificate_print.php?batch_no='.$batch_no);
	?>
    <!--
<center>
<a href="printedCertificate/printAllBatch.php?batch_no=<?php echo $batch_no; ?>">
<input type="button" class="special" value="Print for Batch <?php echo $batch_no; ?>">
</a>
</center>
-->
<?php } ?>
